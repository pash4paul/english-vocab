 ...
 *
 * Словарь американский, поэтому американский голос предпочтителен: британское
 * «can't» и «water» звучат настолько иначе, что на карточке диктанта это
 * превращается в другое упражнение.
 */
export function getEnglishVoice(): SpeechSynthesisVoice | null {
  const voices = englishVoices();
  if (!voices.length) return null;
  if (chosenVoiceKey) {
    const same = voices.find((v) => voiceKey(v) === chosenVoiceKey);
    if (same) return same;
  }
  const american = voices.filter((v) => /^en[-_]us/i.test(v.lang ?? ''));
  const pool = american.length ? american : voices;
  // Локальные голоса не требуют сети — важно для офлайн-режима.
  const pick = pool.find((v) => v.localService) ?? pool[0];
...
export function waitForVoices(timeoutMs = 3000): Promise<SpeechSynthesisVoice | null> {
  ...
  const immediate = getEnglishVoice();
...
export function speak(text: string, rate = 0.85, handlers: SpeakHandlers = {}) {
  ...
  if (!voice) {
    handlers.onError?.('no-english-voice');
    return;
  }
