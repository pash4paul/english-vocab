...
export function Home({
  ...
  return (
    <div className="home">
      ...
      {!tts && progress.settings.enabledKinds.some((k) => k === 'listen' || k === 'spell') && (
        <p className="warn">
          Английский голос в системе не найден — аудирование и диктант пропускаются.
          На macOS: Системные настройки → Универсальный доступ → Устная речь → Системный голос → Добавить → English.
        </p>
      )}
    </div>
  );
