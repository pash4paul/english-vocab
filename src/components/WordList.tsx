...
export function WordList({ deck, progress }: { deck: Deck; progress: Progress }) {
  ...
  return (
    <div className="words">
      ...
      <div className="chips chips-scroll">
        <button className={`chip ${level === '' ? 'on' : ''}`} onClick={() => setLevel('')}>
          Все уровни
        </button>
        {deck.levels.map((l) => (
          ...
          return (
            <li key={word.id} className={`word-row ${st} ${isOpen ? 'open' : ''}`}>
              <button className="word-main" onClick={() => setOpen(isOpen ? null : word.id)}>
                <span className="word-el word-sm">
                  {word.en}
 ...
 * у глаголов прошедшее, у прилагательных сравнительная степень.
 */
function secondForm(w: Word): string | null {
  const f = w.forms;
  if (!f) return null;
  const pick = w.pos === 'noun' ? f.plural : w.pos === 'verb' ? f.past : f.comparative;
  if (!pick) return null;
  const shown = mainForm(pick);
  return shown.toLowerCase() === w.en.toLowerCase() ? null : shown;
}
